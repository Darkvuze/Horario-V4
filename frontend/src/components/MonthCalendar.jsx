...
export default function MonthCalendar({ year, month, employee, codes, region, onCellClick }) {
  ...
  return (
    <div className="panel overflow-visible fz-rise" data-testid="month-calendar">
      ...
      <div className="grid grid-cols-7">
        {cells.map((dayNum, idx) => {
          ...
          const date = startOfDay(new Date(year, month - 1, dayNum));
          ...
          const chipCls = isToday ? "fz-day-chip-today" : isPast ? "fz-day-chip-past" : "fz-day-chip-future";
          ...
          return (
            <button
              ...
              className={`relative h-24 sm:h-28 border-r border-b border-c p-1.5 flex flex-col justify-between text-left ${cellCls} ${isToday ? "fz-today" : ""} hover:opacity-90 active:opacity-75 transition-opacity cursor-pointer`}
            >
              <div className="flex items-start justify-between gap-1">
                <span
                  className={`fz-day-chip ${chipCls}`}
                  data-testid={isToday ? `today-chip-${dayNum}` : isPast ? `past-chip-${dayNum}` : `future-chip-${dayNum}`}
                >
                  {dayNum}
                  {isPast && <X size={10} strokeWidth={3} />}
                </span>
