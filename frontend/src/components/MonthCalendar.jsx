...
export default function MonthCalendar({ year, month, employee, codes, region }) {
        ...
        {cells.map((dayNum, idx) => {
          if (!dayNum) {
            return (
              <div
                key={idx}
                className="h-20 sm:h-24 border-r border-b border-c opacity-30"
          ...
          return (
            <div
              ...
              className={`relative h-20 sm:h-24 border-r border-b border-c p-1.5 flex flex-col justify-between ${cellCls}`}
            >
              <div className="flex items-start justify-between">
                ...
                <div className="flex gap-1">
                  {showAcores && (
                    <span
                      title={hol.name + " (Açores)"}
                      data-testid={`holiday-acores-${dayNum}`}
                      className="inline-block w-2.5 h-2.5 rounded-full bg-sky-500 ring-2 ring-white/80"
                    />
                  )}
                  {showCont && (
                    <span
                      title={hol.name + " (Continental)"}
                      data-testid={`holiday-cont-${dayNum}`}
                      className="inline-block w-2.5 h-2.5 rounded-full bg-pink-500 ring-2 ring-white/80"
                    />
                  )}
                </div>
              </div>
              ...
              {hol && (showAcores || showCont) && (
                <div className="absolute bottom-0.5 left-1 right-1 truncate text-[9px] opacity-70 text-center">
                  {hol.name}
                </div>
              )}
            </div>
